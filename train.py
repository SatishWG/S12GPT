...
class DataLoaderLite:
    def __init__(self, B, T):
        ...
        self.batches_per_epoch = (self.total_tokens - 1) // (B * T)
        print(f'loaded {self.total_tokens} tokens')
    ...
    def next_batch(self):
        ...
        if self.current_position + B * T + 1 > self.total_tokens:
...
while epoch < max_epochs:
    ...
    # Reset data loader position for new epoch
    train_loader.reset()
    ...
    if avg_loss < target_loss:
        print(f"\n✓ Target loss achieved! Final loss: {avg_loss:.6f} < {target_loss}")
        print(f"Training completed in {epoch+1} epochs ({step} steps)")
        break
